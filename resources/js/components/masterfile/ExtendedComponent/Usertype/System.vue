<template>
    <div class="border-x border-b p-2">
        ...
        <div class="border rounded">
            ...
            >
                <tbody class="tbody">
                    <tr class="tr" v-if="!systemUsertype.length">
                        <td colspan="3" class="td text-center">
                            NO DATA AVAILABLE
                        </td>
                    </tr>
                    <tr v-for="(type, i) in systemUsertype" :key="i" class="tr">
...
export default {
    ...
    computed: {
        ...mapState(["errors", "perPage", "pagination2"]),
        ...mapState("Usertype", ["systemUsertype"])
    },
    methods: {
        ...mapActions("Usertype", [
            "getUserType",
        ...
        update() {
            ...
            this.updateSystemUsertype(payload);
        },
        create() {
            let payload = {
                usertype: this.form.usertype,
            ...
            this.addSystemUsertype(payload);
        },
    ...
    mounted() {
        Fire.$on("reload_system_usertype", () => {
            this.fetch();
            this.drawer = false;
        });
        this.fetch();
