...
export default {
    props: ["flag", "bunit_code", "drawer", "url"],
    data() {
        return {
            image: null,
            preview: null
    ...
    methods: {
        ...mapActions("B_unit", ["uploadPhoto"]),

        upload() {
            let form = new FormData();
            form.append("image", this.image);
            form.append("id", this.bunit_code);
            this.uploadPhoto(form);
        },
        selectFile() {
            document.getElementById("fileChooser2").click();
        },
