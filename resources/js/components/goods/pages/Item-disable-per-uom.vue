<template>
    <div class="space-y-2 ">
        <module-header
            ...
            title="Disable Product Unit of Measure"
        />
        <div class="border p-2 space-y-2">
            ...
            <div class="mb-1 " v-if="form.itemIds.length != 0">
                <Tooltip content="Disable" placement="right">
                    <Button @click="disabledAll" type="error" icon="md-trash" />
                </Tooltip>
            </div>
            <div class="border rounded">
                <table class="min-w-full">
                    ...
                    <tbody class="tbody ">
                        <tr class="tr" v-if="!DisableProducts.length">
                            <td colspan="6" class="td text-center ">
                                NO DATA AVAILABLE
                            </td>
                        </tr>
                        <tr
                            v-for="(item, i) in DisableProducts"
                            :key="i"
                            class="tr"
                        >
...
export default {
    ...
    computed: {
        ...
        ...mapState("Product", ["DisableProducts"])
    },
    methods: {
        ...mapActions(["getItemCategory"]),
        ...mapActions("Product", ["getDisabledProducts", "disableProduct"]),
        nextPage() {
            this.currentPage++;
            this.fetch();
        },
        ...
        disabledAll() {
            this.$Modal.confirm({
                title: "Disable",
                content: "<p>Do you want to  disable this product uom?</p>",
                okText: "OK",
                cancelText: "Cancel",
                onOk: () => {
                    this.disableProduct({ ids: this.form.itemIds });
                },
                onCancel: () => {
        ...
        selectAll() {
            this.form.itemIds = [];
            let products = this.DisableProducts;
            let item;
            if (!this.allSelected) {
    ...
    mounted() {
        Fire.$on("reload_disabled_product", () => {
            this.fetch();
            this.reset();
        });
        this.fetch();
