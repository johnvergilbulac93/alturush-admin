<template>
    <div class="space-y-2">
        <module-header icon="md-cloud-upload" title="Upload Product Category" />
    ...
    methods: {
        handleFileChange(event) {
            ...
            if (validExtensions.includes(fileNameExt)) {
                ...
                this.product = null;
                document.getElementById("product_category_file").value = "";
            }
        },
        reset() {
            this.loading = false;
        ...
        async upload() {
            ...
            payload.append("file_category", this.product);
            ...
            try {
                NProgress.start();
                this.loading = true;
                const { status, data } = await Http.upload_category(
                    payload,
                    source
                );
                if (status == 200) {
