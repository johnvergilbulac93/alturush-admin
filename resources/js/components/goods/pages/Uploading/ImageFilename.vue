<template>
    <div class="space-y-2">
        <module-header icon="md-cloud-upload" title="Upload Image Filename" />
    ...
    methods: {
        handleFileChange(event) {
            ...
            if (validExtensions.includes(fileNameExt)) {
                ...
                this.product = null;
                document.getElementById("image_name_file").value = "";
            }
        },
        reset() {
            this.loading = false;
        ...
        async upload() {
            ...
            payload.append("image_filename", this.product);
            ...
            try {
                NProgress.start();
                this.loading = true;
                const { status, data } = await Http.upload_image_filename(
                    payload,
                    source
                );
                if (status == 200) {
