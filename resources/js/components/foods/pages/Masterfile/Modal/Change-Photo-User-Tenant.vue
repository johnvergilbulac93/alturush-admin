...
export default {
    ...
    watch: {
        drawer() {
            this.preview = null;
            this.image = null;
            document.getElementById("tenant_file").value = "";
        }
    },
    methods: {
        ...mapActions("User", ["updateTenantUserPhoto"]),

        upload() {
            let form = new FormData();
            form.append("image", this.image);
            form.append("id", this.id);
            this.updateTenantUserPhoto(form);
        },
        selectFile() {
            document.getElementById("tenant_user_file").click();
        },
        handleChangeImage(event) {
            ...
            if (validExtensions.includes(fileNameExt)) {
                ...
                this.image = null;
                document.getElementById("tenant_user_file").value = "";
            }
        }
    }
};
